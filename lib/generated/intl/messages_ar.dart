// DO NOT EDIT. This is code generated via package:intl/generate_localized.dart
// This is a library that provides messages for a ar locale. All the
// messages from the main program should be duplicated here with the same
// function name.

// Ignore issues from commonly used lints in this file.
// ignore_for_file:unnecessary_brace_in_string_interps, unnecessary_new
// ignore_for_file:prefer_single_quotes,comment_references, directives_ordering
// ignore_for_file:annotate_overrides,prefer_generic_function_type_aliases
// ignore_for_file:unused_import, file_names, avoid_escaping_inner_quotes
// ignore_for_file:unnecessary_string_interpolations, unnecessary_string_escapes

import 'package:intl/intl.dart';
import 'package:intl/message_lookup_by_library.dart';

final messages = new MessageLookup();

typedef String MessageIfAbsent(String messageStr, List<dynamic> args);

class MessageLookup extends MessageLookupByLibrary {
  String get localeName => 'ar';

  final messages = _notInlinedMessages(_notInlinedMessages);
  static Map<String, Function> _notInlinedMessages(_) => <String, Function>{
    "AboutUs": MessageLookupByLibrary.simpleMessage("معلومات عنا"),
    "Account": MessageLookupByLibrary.simpleMessage("الحساب"),
    "Add": MessageLookupByLibrary.simpleMessage("إضافة"),
    "AddFreinds": MessageLookupByLibrary.simpleMessage("أضف أصدقاء"),
    "AddFriend": MessageLookupByLibrary.simpleMessage("أضف صديقًا"),
    "AddOne": MessageLookupByLibrary.simpleMessage("أضف واحدة!"),
    "AhmedAwadNosier": MessageLookupByLibrary.simpleMessage("أحمد عوض نصير"),
    "ApplyFilter": MessageLookupByLibrary.simpleMessage("تطبيق التصفية"),
    "Arabic": MessageLookupByLibrary.simpleMessage("العربية"),
    "AreYousureyouwanttodelleteyouraccout":
        MessageLookupByLibrary.simpleMessage(
          "هل أنت متأكد أنك تريد حذف الحساب؟",
        ),
    "Areyoursureyouwantdelletethisnote": MessageLookupByLibrary.simpleMessage(
      "هل أنت متأكد أنك تريد حذف هذه الملاحظة؟",
    ),
    "Areyoursureyouwantdiscardyourchanges":
        MessageLookupByLibrary.simpleMessage(
          "هل أنت متأكد أنك تريد تجاهل التغييرات؟",
        ),
    "Asaprofessionaldeveloper": MessageLookupByLibrary.simpleMessage(
      "بصفتي مطورًا محترفًا، أتابع مستجدات نظام Flutter، أساهم في حل المشكلات، وأهدف دائمًا إلى تقديم تطبيقات تحقق التوازن بين الجودة، قابلية التوسع، والقيمة التجارية.",
    ),
    "BestSeller": MessageLookupByLibrary.simpleMessage("الأكثر مبيعًا"),
    "BeyondjustFlutterIunderstandhowtointegratebackendservices":
        MessageLookupByLibrary.simpleMessage(
          "إلى جانب Flutter، أفهم كيفية دمج خدمات الخلفية (REST APIs، Firebase، GraphQL)، إدارة المصادقة الآمنة، التعامل مع قواعد البيانات المحلية والبعيدة، وتنفيذ تصاميم واجهة مستخدم حديثة مع مراعاة سهولة الاستخدام وتعدد اللغات.",
        ),
    "BookType": MessageLookupByLibrary.simpleMessage("نوع الكتاب"),
    "Business": MessageLookupByLibrary.simpleMessage("الأعمال"),
    "Cancel": MessageLookupByLibrary.simpleMessage("إلغاء"),
    "Cloud": MessageLookupByLibrary.simpleMessage("سحابة"),
    "Confirm": MessageLookupByLibrary.simpleMessage("تأكيد"),
    "ContactInformation": MessageLookupByLibrary.simpleMessage(
      "معلومات التواصل",
    ),
    "ContactUs": MessageLookupByLibrary.simpleMessage("تواصل معنا"),
    "Content": MessageLookupByLibrary.simpleMessage("المحتوى"),
    "Customersatisfactionismytoppriority": MessageLookupByLibrary.simpleMessage(
      "- رضا العملاء هو أولويتي القصوى",
    ),
    "DarkMode": MessageLookupByLibrary.simpleMessage("الوضع المظلم"),
    "Dellete": MessageLookupByLibrary.simpleMessage("حذف"),
    "DelleteAccount": MessageLookupByLibrary.simpleMessage("حذف الحساب"),
    "Discard": MessageLookupByLibrary.simpleMessage("تجاهل"),
    "Download": MessageLookupByLibrary.simpleMessage("تنزيل"),
    "English": MessageLookupByLibrary.simpleMessage("الإنجليزية"),
    "Entertainment": MessageLookupByLibrary.simpleMessage("الترفيه"),
    "Filter": MessageLookupByLibrary.simpleMessage("تصفية"),
    "FollowUs": MessageLookupByLibrary.simpleMessage("تابعنا"),
    "ForgotPassword": MessageLookupByLibrary.simpleMessage("نسيت كلمة السر؟"),
    "FreePreview": MessageLookupByLibrary.simpleMessage("معاينة مجانية"),
    "FreindRequsts": MessageLookupByLibrary.simpleMessage("طلبات الصداقة"),
    "GetInTouch": MessageLookupByLibrary.simpleMessage("ابقَ على تواصل"),
    "Health": MessageLookupByLibrary.simpleMessage("الصحة"),
    "Hi": MessageLookupByLibrary.simpleMessage("اهلا, "),
    "IamaSoftwareEngineerspecializedinmobiledevelopmentwithFlutter":
        MessageLookupByLibrary.simpleMessage(
          "أنا مهندس برمجيات متخصص في تطوير تطبيقات الهواتف باستخدام Flutter. خبرتي تكمن في بناء تطبيقات عالية الجودة، قابلة للتوسع، وسريعة الأداء لتقديم تجربة مستخدم سلسة على كل من أندرويد و iOS.",
        ),
    "Ihavestrongfoundationsinsoftwareengineeringprinciples":
        MessageLookupByLibrary.simpleMessage(
          "لدي أساس قوي في مبادئ هندسة البرمجيات، بما في ذلك الهندسة النظيفة، إدارة الحالة، تحسين الأداء، والاختبارات. ألتزم بأفضل الممارسات مثل مبادئ SOLID، أنماط التصميم، ودمج CI/CD لضمان كود قابل للصيانة وجاهز للإنتاج.",
        ),
    "Istriveforcontinousimprovment": MessageLookupByLibrary.simpleMessage(
      "- أسعى باستمرار نحو التطوير والتحسين",
    ),
    "Ivaluehonestyandintegrityinallmyactions":
        MessageLookupByLibrary.simpleMessage(
          "- أقدّر الصدق والنزاهة في جميع أفعالي",
        ),
    "Language": MessageLookupByLibrary.simpleMessage("اللغة"),
    "Login": MessageLookupByLibrary.simpleMessage("تسجيل الدخول"),
    "Loginwithgoogle": MessageLookupByLibrary.simpleMessage("سجل دخول بجوجل"),
    "Messages": MessageLookupByLibrary.simpleMessage("الرسائل"),
    "Name": MessageLookupByLibrary.simpleMessage("الاسم"),
    "News": MessageLookupByLibrary.simpleMessage("الأخبار"),
    "NewsCategroy": MessageLookupByLibrary.simpleMessage("فئة الأخبار"),
    "NewsLanguage": MessageLookupByLibrary.simpleMessage("لغة الأخبار"),
    "NoFriendsYetgoMakeSomeFreinds": MessageLookupByLibrary.simpleMessage(
      "لا أصدقاء بعد! اذهب وكون بعض الأصدقاء؟",
    ),
    "NoInternetConnection": MessageLookupByLibrary.simpleMessage(
      "لا يوجد اتصال بالإنترنت",
    ),
    "NoMessagesFound": MessageLookupByLibrary.simpleMessage(
      "لم يتم العثور على رسائل؟",
    ),
    "NoNotesFound": MessageLookupByLibrary.simpleMessage(
      "لم يتم العثور على ملاحظات؟",
    ),
    "NoUsersYet": MessageLookupByLibrary.simpleMessage("لا يوجد مستخدمون بعد!"),
    "Off": MessageLookupByLibrary.simpleMessage("إيقاف"),
    "On": MessageLookupByLibrary.simpleMessage("تشغيل"),
    "Open": MessageLookupByLibrary.simpleMessage("فتح"),
    "OrderBy": MessageLookupByLibrary.simpleMessage("ترتيب حسب"),
    "Phone": MessageLookupByLibrary.simpleMessage("الهاتف"),
    "Pleasecheckyourinternetsettingsandtryagain":
        MessageLookupByLibrary.simpleMessage(
          "يرجى التحقق من إعدادات الإنترنت الخاصة بك والمحاولة مرة أخرى.",
        ),
    "PleasecheckyourtimesettingsandmakesuretheAMPMiscorrect":
        MessageLookupByLibrary.simpleMessage(
          "يرجى تصحيح الوقت (ص/م) ثم المحاولة مرة أخرى",
        ),
    "PriceType": MessageLookupByLibrary.simpleMessage("نوع السعر"),
    "Save": MessageLookupByLibrary.simpleMessage("  حفظ "),
    "Savechanges": MessageLookupByLibrary.simpleMessage(
      "هل تريد حفظ التغييرات ؟",
    ),
    "Science": MessageLookupByLibrary.simpleMessage("العلوم"),
    "SearchbyName": MessageLookupByLibrary.simpleMessage("ابحث بالاسم..."),
    "SendMessage": MessageLookupByLibrary.simpleMessage("إرسال"),
    "Settings": MessageLookupByLibrary.simpleMessage("الإعدادات"),
    "SignOut": MessageLookupByLibrary.simpleMessage("تسجيل الخروج"),
    "Sports": MessageLookupByLibrary.simpleMessage("الرياضة"),
    "StartNewChat": MessageLookupByLibrary.simpleMessage("ابدأ محادثة جديدة!"),
    "Tapeyourmessage": MessageLookupByLibrary.simpleMessage("اكتب رسالتك"),
    "Thisusernolongerexists": MessageLookupByLibrary.simpleMessage(
      "هذا المستخدم لم يعد موجودا",
    ),
    "Title": MessageLookupByLibrary.simpleMessage("العنوان"),
    "Typesomething": MessageLookupByLibrary.simpleMessage("اكتب شيئًا..."),
    "Values": MessageLookupByLibrary.simpleMessage("القيم :"),
    "Welcometoourapp": MessageLookupByLibrary.simpleMessage(
      "أهلاً بيك في تطبيقنا!",
    ),
    "Wesentressetpasswordemailtoyouremailboxgoandressetyourpassword":
        MessageLookupByLibrary.simpleMessage(
          "لقد أرسلنا رسالة لإعادة تعيين كلمة المرور إلى بريدك الإلكتروني. يرجى التحقق من بريدك الإلكتروني وإعادة تعيين كلمة المرور.",
        ),
    "WhoIm": MessageLookupByLibrary.simpleMessage("من أنا؟ :"),
    "YouCanAlsoLike": MessageLookupByLibrary.simpleMessage("قد يعجبك أيضًا"),
    "Youremailisnotverifiedgotoemialboxandverifyyouremail":
        MessageLookupByLibrary.simpleMessage(
          "بريدك الإلكتروني غير مُفعّل. يرجى التحقق من بريدك الإلكتروني لتفعيل الحساب",
        ),
    "biography": MessageLookupByLibrary.simpleMessage("سيرة ذاتيّة"),
    "books": MessageLookupByLibrary.simpleMessage("كتب"),
    "chat": MessageLookupByLibrary.simpleMessage("دردشة"),
    "chatwithfriends": MessageLookupByLibrary.simpleMessage("تحدث مع الأصدقاء"),
    "children": MessageLookupByLibrary.simpleMessage("أطفال"),
    "choosprofilephoto": MessageLookupByLibrary.simpleMessage("اختر صوره ملفك"),
    "createyourownaccount": MessageLookupByLibrary.simpleMessage(
      "أنشئ حسابك الخاص!",
    ),
    "ebooks": MessageLookupByLibrary.simpleMessage("كتب إلكترونيّة"),
    "email": MessageLookupByLibrary.simpleMessage("البريد الإلكتروني"),
    "entertheemailaddressyouused": MessageLookupByLibrary.simpleMessage(
      "أدخل عنوان البريد الإلكتروني الذي استخدمته عند التسجيل وسنرسل لك تعليمات لإعادة تعيين كلمة المرور.",
    ),
    "enteryouremail": MessageLookupByLibrary.simpleMessage(
      "أدخل بريدك الإلكتروني",
    ),
    "explorebooks": MessageLookupByLibrary.simpleMessage("استكشف الكتب"),
    "fantasy": MessageLookupByLibrary.simpleMessage("خيال"),
    "fieldisrequired": MessageLookupByLibrary.simpleMessage("الحقول دي مطلوبة"),
    "firstname": MessageLookupByLibrary.simpleMessage("الاسم الأول"),
    "freeebooks": MessageLookupByLibrary.simpleMessage("كتب إلكترونيّة مجانية"),
    "general": MessageLookupByLibrary.simpleMessage("عام"),
    "historicalfiction": MessageLookupByLibrary.simpleMessage(
      "روايات تاريخيّة",
    ),
    "ifyoudonothaveaccount": MessageLookupByLibrary.simpleMessage(
      " إذا لم يكن لديك حساب؟",
    ),
    "ifyouhaveaccount": MessageLookupByLibrary.simpleMessage(
      "إذا كان لديك حساب،",
    ),
    "keep": MessageLookupByLibrary.simpleMessage("  احتفاظ "),
    "lastname": MessageLookupByLibrary.simpleMessage("اسم العائلة"),
    "message": MessageLookupByLibrary.simpleMessage("رسالة"),
    "myEmail": MessageLookupByLibrary.simpleMessage(
      "ahmedawadnosier@gmail.com",
    ),
    "myPhoneNumber": MessageLookupByLibrary.simpleMessage("+٠١٢٧٣٨٥٦٩١٢"),
    "mystery": MessageLookupByLibrary.simpleMessage("روايات غموض"),
    "newest": MessageLookupByLibrary.simpleMessage("الأحدث"),
    "news": MessageLookupByLibrary.simpleMessage("أخبار"),
    "notes": MessageLookupByLibrary.simpleMessage("ملاحظات"),
    "onbaordingNewssubtitle": MessageLookupByLibrary.simpleMessage(
      "تابع آخر الأخبار، العناوين العاجلة، والمقالات أولاً بأول",
    ),
    "onbaordingNewstitle": MessageLookupByLibrary.simpleMessage(
      "ابقَ مطلعًا، وكن في المقدمة",
    ),
    "onbaordingbookssubtitle": MessageLookupByLibrary.simpleMessage(
      "استكشف عالماً من القصص والمعرفة والإلهام مع آلاف الكتب الإلكترونية والكتب الصوتية في تطبيق سهل الاستخدام",
    ),
    "onbaordingbookstitle": MessageLookupByLibrary.simpleMessage(
      "اقرأ، اكتشف، اهرب",
    ),
    "onbaordingchatsubtitle": MessageLookupByLibrary.simpleMessage(
      "ارسل رسائل فورية إلى الأصدقاء والعائلة أو الفرق بالنصوص والصور والصوت — في أي وقت وأي مكان",
    ),
    "onbaordingchattitle": MessageLookupByLibrary.simpleMessage(
      "تواصل، شارك، تحدث",
    ),
    "onbaordingnotessubtitle": MessageLookupByLibrary.simpleMessage(
      "قم بتدوين الأفكار والتذكيرات وقوائم المهام بسرعة للحفاظ على تنظيم حياتك وأفكارك في مكان واحد",
    ),
    "onbaordingnotestitle": MessageLookupByLibrary.simpleMessage(
      "سجّل، نظّم، تذكّر",
    ),
    "paidebooks": MessageLookupByLibrary.simpleMessage("كتب إلكترونيّة مدفوعة"),
    "password": MessageLookupByLibrary.simpleMessage("كلمة السر"),
    "popularity": MessageLookupByLibrary.simpleMessage("الشعبية"),
    "programing": MessageLookupByLibrary.simpleMessage("البرمجة"),
    "publishedAt": MessageLookupByLibrary.simpleMessage("تاريخ النشر"),
    "relevance": MessageLookupByLibrary.simpleMessage("الأكثر صلة"),
    "relevancy": MessageLookupByLibrary.simpleMessage("الملاءمة"),
    "requestpasswordresset": MessageLookupByLibrary.simpleMessage(
      "طلب إعادة تعيين كلمة المرور",
    ),
    "resetPassword": MessageLookupByLibrary.simpleMessage(
      "إعادة تعيين كلمة المرور",
    ),
    "romance": MessageLookupByLibrary.simpleMessage("رومانسيّات"),
    "sciencefiction": MessageLookupByLibrary.simpleMessage("خيال علمي"),
    "selfhelp": MessageLookupByLibrary.simpleMessage("تطوير الذات"),
    "signUp": MessageLookupByLibrary.simpleMessage("سجّل حساب"),
    "skip": MessageLookupByLibrary.simpleMessage("تخطي"),
    "takenotes": MessageLookupByLibrary.simpleMessage("دون ملاحظات"),
    "technology": MessageLookupByLibrary.simpleMessage("التكنولوجيا"),
    "truecrime": MessageLookupByLibrary.simpleMessage("جريمة حقيقية"),
    "userName": MessageLookupByLibrary.simpleMessage("اسم المستخدم"),
    "whathapend": MessageLookupByLibrary.simpleMessage("ماذا يحدث"),
    "youngadult": MessageLookupByLibrary.simpleMessage("شبابي"),
    "yourememberyourpassword": MessageLookupByLibrary.simpleMessage(
      "هل تتذكر كلمة المرور الخاصة بك؟",
    ),
  };
}
